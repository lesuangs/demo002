...
export default class GamesUtils {
   ...
   * @returns {*[]}
   */
  static setSSCMethodInit(data, data1, lotteryId, userRebate, inputRebate) {
  ...
  static setLuckyTreeMethodInit(data, lotteryId, data1, userRebate, inputRebate) {
    ...
    navList.map((item, key) => {
      if (Number(item.code) > Number(`${lotteryId}105`)) navList.splice(key, 1)
      // 点数数据排序纠正，3点放在第一位
      if (key === 2) {
