  ...
  export default {
    ...
    methods: {
      ...
      setJson(data) {
        ...
        let xyMenu = GamesUtils.setLucky28MethodInit(data, lotteryId, userRebate, inputRebate);
        this.betList = xyMenu
        this.simulation()
        sessionStorage.setItem(`pcdd${lotteryId}`, JSON.stringify(this.betList))
      },
      // 初次加载默认数据
      simulation() {
    ...
    created() {
      ...
      if (sessionStorage.getItem(`pcdd${this.lotteryId}`)) {
        this.betList = JSON.parse(sessionStorage.getItem(`pcdd${this.lotteryId}`))
        this.simulation()
      } else {
        this.getList()
      }
