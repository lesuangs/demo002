...
export default {
  ...
  methods: {
    ...
    setJson(data, data1) {
      ...
      let newVar = GamesUtils.setBjkl8MethodInit(data, lotteryId, data1, userRebate, inputRebate);
      // 玩法整合
      this.betList = newVar
      // 菜单加载
      this.simulation()
      sessionStorage.setItem(`bjkl8${lotteryId}`, JSON.stringify(this.betList))
    },
    // 初次加载默认数据
    simulation() {
    ...
    // 自定义号码
    mapArr(ids, key) {
      let singleArr = Array.from({length: 80})
  ...
  created() {
    ...
    if (sessionStorage.getItem(`bjkl8${this.lotteryId}`)) {
      this.betList = JSON.parse(sessionStorage.getItem(`bjkl8${this.lotteryId}`))
      this.simulation()
    } else {
      this.getList()
    }
