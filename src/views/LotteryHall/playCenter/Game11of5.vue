...
export default {
  ...
  methods: {
    ...
    setJson(data, data1) {
      ...
      let newVar = GamesUtils.setGame11of5fMethodInit(data, lotteryId, data1, userRebate, inputRebate);
      //玩法整合
      this.betList = newVar
      sessionStorage.setItem(`11of5${lotteryId}`, JSON.stringify(this.betList))
      this.simulation()
    },
    // 初次加载默认数据
    simulation() {
      ...
      // 二级菜单渲染
      if (this.firstSelected === 2) this.$EventBus.$emit('isSecond')
    },
    // 菜单选择
    seletClick(arr) {
      this.firstSelected = arr[0] // 一级菜单处理
  ...
  created() {
    ...
    if (sessionStorage.getItem(`11of5${this.lotteryId}`)) {
      this.betList = JSON.parse(sessionStorage.getItem(`11of5${this.lotteryId}`))
      this.simulation()
    } else {
      this.getList()
    }
