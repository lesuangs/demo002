...
export default {
  ...
  methods: {
    ...
    setJson(data) {
      ...
      let newVar = GamesUtils.setGxklsfMethodInit(data, lotteryId, userRebate, inputRebate);
      // 玩法整合
      this.betList = newVar
      // 菜单加载
      this.simulation()
      sessionStorage.setItem(`gxklsf${lotteryId}`, JSON.stringify(this.betList))
    },
    // 初次加载默认数据
    simulation() {
  ...
  created() {
    ...
    if (sessionStorage.getItem(`gxklsf${this.lotteryId}`)) {
      this.betList = JSON.parse(sessionStorage.getItem(`gxklsf${this.lotteryId}`))
      this.simulation()
    } else {
      this.getList()
    }
